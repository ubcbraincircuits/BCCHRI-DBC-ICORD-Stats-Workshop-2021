...
# Btw, what was our actual sample mean? Use the mean() function. 
...
distribution have? #!!!!# what does this mean? 
...
#resamples. store the resulting distribution in a variable called dist
...
# Save the "t" value of the output (the distribution) using output$t in a variable named dist
dist = output$t
```
...
avg_RS_se=sd(avg_RS)
# Using the bootstrap distribution from boot, call it dist_se. 
dist_se=sd(dist)
```
...
# traditional t-confidence interval is stat +/- t*SE_stat 
# for alpha=0.05;
alpha=0.05
tcrit = qt(1-alpha/2,1550) #!!!# wait a minute where did this input come from? 
mean(dist)-tcrit*sd(dist)
mean(dist)+tcrit*sd(dist)
...
print(length(which(dist>per_ci$percent[,4] & dist < per_ci$percent[,5]))/length(dist))
...
# Call your resamples x1_RS and x2_RS
...
# Calculate the difference between all of our means. #!!!# does the order matter?
