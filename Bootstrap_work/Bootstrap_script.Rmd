...
```{r}
load("dat.rda")
#this is something non-gaussian that I made up.  Meant to mimic a non-normal data set with 1550 data 
#points. Let's take a look at it using hist.
hist(dat,breaks=30)
```
...
# found like this:
ind=sample.int(1550,1550,replace=TRUE) ##!!!### Please remeber to mention what an index is real quick. 
...
```
Before we get into Permutation testing, let's modify our make_resamples so that we can get resamples with a specific number of data points.  Start by saving your function with a new name make_samples2.  Don't forget to rename the function.  Add an extra input argument called nsamp which will be the number of data points in the resample.  
...
# loop starts at one and goes to nresamps
    ...
    for (i in 1:nresamps) {
    #generate random numbers for resample
    ind=sample.int(length(dat),nsamps,replace=TRUE) #!!!# Wait, are we doing replacement here? 
    # index data with new indices
    dat_RS=dat[ind]
    # store in output matrix
    resamps_out[i,]=dat_RS
...
x1=2*rnorm(27)+1 #!!!# just set the mean and sd. 
x2=3*rnorm(47)+2
...
# 27 numel(x1) & it should be 47 (numel(x2)) for x2

x1_RS=make_resamples2(x1,1,length(x1))
x2_RS=make_resamples2(x2,1,length(x2))
...
# Second dim.
x1_RS_avg=apply(x_RS,1,mean)
x2_RS_avg=apply(x_RS,1,mean)
